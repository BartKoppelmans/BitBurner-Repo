...
import {
	...
	LONG_TERM_FORECAST_WINDOW_LENGTH,
...
export class EarlyStockManager extends StockManager {
	...
	protected preDetectPossibleInversion(ns: NS, stock: Stock): boolean {
		const nearTermForecast: number    = EarlyStockManager.calculateForecast(stock.priceHistory.slice(0, NEAR_TERM_FORECAST_WINDOW_LENGTH))
		const preNearTermForecast: number = EarlyStockManager.calculateForecast(stock.priceHistory.slice(NEAR_TERM_FORECAST_WINDOW_LENGTH))
	...
	protected shouldBuy(stock: Stock): boolean {
		...
		if (stock.stockForecastInformation.tools.lastInversion < PRICE_HISTORY_THRESHOLD) return false
