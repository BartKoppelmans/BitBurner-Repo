...
export default abstract class BaseStockManager implements Manager {
	...
	public async start(ns: NS): Promise<void> {
		if (this.stockStorage.totalTicks === 0) {
			...
			const stockInformationDict: StockInformationDict = await BaseStockManager.getStockInformation(ns, this.stockStorage)
			this.stockStorage.updateStockInformation(stockInformationDict)

			const stockForecastDict: StockForecastDict = this.getStockForecast(ns, this.stockStorage, {  })
	...
	public setStockStorage(stockStorage: StockStorage): void {
		this.stockStorage = stockStorage
	}
	public getStockStorage(): StockStorage {
	...
	private async buyStocks(ns: NS): Promise<StockPurchase[]> {
		...
		for (const stock of prioritizedStocks) {
			...
			if (remainingStockBudget) budget = Math.min(budget, remainingStockBudget)
			...
			let numShares: number = Math.floor((budget - STOCK_CONSTANT.STOCK_COMMISSION) / price)
	...
	private printSummary(ns: NS): void {
		...
		const liquidationValue: number = ownedStocks.reduce((total, stock) => total - STOCK_CONSTANT.STOCK_COMMISSION + stock.getValue(), 0)
	...
	private getStockForecast(ns: NS, stockStorage: StockStorage, stockInversionDict: StockInversionDict): StockForecastDict {
		...
		for (const stock of stockStorage.stocks) {
			...
			} else if (stock.stockForecastInformation?.tools?.lastInversion) stock.stockForecastInformation.tools.lastInversion++
