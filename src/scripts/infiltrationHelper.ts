...
export let
    ...
    pxToNum = (input: string) => Number(input.replaceAll("px", "")),
    sleep = (ms: number | undefined) => new Promise(resolve => setTimeout(resolve, ms)),
    enableEdgeDetection = async (el: HTMLDivElement) => {
    ...
    pressKey = (key: string) => {
        var event: KeyboardEvent = new KeyboardEvent('keydown', { 'key': key, bubbles: true });
        document.activeElement!.dispatchEvent(event);
    },
    pressKeyCode = (keyCode: number, hasShift: boolean) => {

        var event: KeyboardEvent = new KeyboardEvent('keydown', { keyCode: keyCode, shiftKey: hasShift, bubbles: true });
        document.activeElement!.dispatchEvent(event);
    },
    createWindow = (prefix: string, title: string, mainContent: string) => {
    ...
    parseMineField = (container: HTMLDivElement): boolean[][] => {
        const rows: HTMLDivElement[] = Array.from(container.childNodes).slice(1, container.childNodes.length - 1) as HTMLDivElement[];
        ...
        let matrix: boolean[][] = [...Array(numRows)].map(e => Array(numColumns).fill(0));
    ...
    parseCyberpunkField = (elements: HTMLSpanElement[]): string[][] => {

        const dimensions: number = Math.sqrt(elements.length);

        let matrix: string[][] = [...Array(dimensions)].map(e => {
    ...
    colorStringToColor = (s: string): string => {
        switch (s) {
            case "red":
                return "red";
            case "yellow":
            ...
            case "white":
                return "white";
            default:
                console.log(`We did not recognize the color ${s}`);
                return "";
        }
    },
    // --- Main function --- //
    main = async (ns: NS) => {
        let
            // --- Declarations --- //
            infilContainer = document.querySelector("#infiltration-container") as HTMLDivElement,
            mainContent = `<p style="font-size:11px">Assistance:<input type=checkbox class=checkbox id="infil-assistance-toggle" checked /></p><p id="infil-helper-content"></p>`,
            infilWin = createWindow("infil", "Infiltration Helper", mainContent),
            helperContent = infilWin.querySelector("#infil-helper-content"),
            assistanceToggle = infilWin.querySelector("#infil-assistance-toggle") as HTMLInputElement,
            changeContent = (content: string) => (helperContent as HTMLDivElement).innerHTML = content,
            getGameText = () => {
                try { return (infilContainer.children[0].children[1].children[0].children[1].children[0] as HTMLDivElement).innerText; }
                catch (e) { return null; }
            };
        // --- Main Loop --- //
        while (document.body.contains(infilWin)) {
            while (getGameText() && assistanceToggle.checked) {
                switch (getGameText()) {
                    case "Remember all the mines!":
                        let minefield = infilContainer.children[0].children[1].children[0].children[1] as HTMLDivElement;
                        const matrix: boolean[][] = parseMineField(minefield);

                        const rowLength: number = matrix[0].length;

                        let currentLocation: { x: number, y: number; } = { x: 0, y: 0 };
                    ...
                    case "Type it backward":
                        let fullTypingChallenge = (infilContainer.querySelector(`[style="transform: scaleX(-1);"]`) as HTMLDivElement).innerText;
                    ...
                    case "Say something nice about the guard.":
                        ...
                        while (getGameText() == "Say something nice about the guard.") {
                            let isNice = (niceWords.includes((infilContainer.querySelectorAll(`h2[style="font-size: 2em;"]`)[1] as HTMLDivElement).innerText));
                    ...
                    case "Match the symbols!":
                        ...
                        const targetStrings: string[] = targets.filter((element) => element.innerText != null).map((element) => element.innerText.replace(/\s/g, ''));
                    ...
                    case "Cut the wires with the following properties!":
                        ...
                        const wireElements: HTMLSpanElement[] = Array.from(infilContainer.children[0].children[1].querySelectorAll("span")).filter((element) => isNaN(+element.innerText)) as HTMLSpanElement[];

                        const numWires: number = Array.from(infilContainer.children[0].children[1].querySelectorAll("span")).filter((element) => !isNaN(+element.innerText)).length;

                        const wires: HTMLSpanElement[][] = [...Array(numWires)].map(e => {
                            return [];
                        });
                        ...
                        let numberTargets: number[] = [];
                        let colorTargets: string[] = [];
                        ...
                        for (const wireTargetElement of wireTargetElements) {
                            ...
                            if (numberMatch) {
                                numberTargets.push(parseInt(numberMatch[1]));
                            } else if (colorMatch) {
                                colorTargets.push(colorStringToColor(colorMatch[1]));
                            } else {
                                console.log(`We did not recognize element ${wireTargetElement}`);
                            }
                        }
                        ...
                        for (const number of numberTargets) {
                            pressKey(number.toString());
                        }
                        ...
                        while (getGameText() == "Enter the Code!") {
                            ...
                            switch (arrow) {
                                ...
                                case '↓':
                                    ...
                                    break;
                                default:
                                    console.log(`We did not recognize arrow ${arrow}`);
                            }
                        }
                        break;
                        ...
                        for (const bracket of openingBrackets.reverse()) {
                            ...
                            switch (bracket) {
                                ...
                                case "<":
                                    ...
                                    break;
                                default:
                                    console.log(`We didn't recognize bracket '${bracket}'`);
                                    break;
                            }
                        }
