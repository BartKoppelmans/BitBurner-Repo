...
export default class BatchJobUtils {
    ...
    static async scheduleCycle(ns, target, batchStart) {
        const scheduledJob1 = await this.createCycleJob(ns, target, Tools.HACK, batchStart);
        let scheduledJobStart2 = new Date(scheduledJob1.end.getTime() + CONSTANT.CYCLE_DELAY);
        const scheduledJob2 = await this.createCycleJob(ns, target, Tools.WEAKEN, scheduledJobStart2, true);
        let scheduledJobStart3 = new Date(scheduledJob2.end.getTime() + CONSTANT.CYCLE_DELAY);
        const scheduledJob3 = await this.createCycleJob(ns, target, Tools.GROW, scheduledJobStart3);
        let scheduledJobStart4 = new Date(scheduledJob3.end.getTime() + CONSTANT.CYCLE_DELAY);
        const scheduledJob4 = await this.createCycleJob(ns, target, Tools.WEAKEN, scheduledJobStart4, false);
        return [scheduledJob1, scheduledJob2, scheduledJob3, scheduledJob4];
    }
    static async createCycleJob(ns, target, tool, start, isFirstWeaken = false) {
        let threads;
        let executionTime;
    ...
    static getOptimalBatchCost(ns, target) {
