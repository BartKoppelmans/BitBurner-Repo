...
export let main = async (ns) => {
    let mainContent = `<p class=small><button id="css-load-button">Load</button> <button id="css-save-button">Save</button> <button id="css-load1-button">Refresh</button> <button id="css-save1-button">Preview</button> <button id="css-minify-button">Minify</button> <button id="css-beautify-button">Beautify</button></p><textarea spellcheck=false id="css-textarea"></textarea>`, cssBox = createBox("CSS Editor", mainContent), cssElem = doc.querySelector(`#boxCSS`), cssText = cssBox.querySelector('textarea'), loadFromFile = () => cssText.value = ns.read("/API/box/css.ns").replace(/^export let css=`|`;$/g, "");
