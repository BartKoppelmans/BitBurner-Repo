...
pxToNum = (input) => Number(input.replaceAll("px", "")), sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms)), enableEdgeDetection = async (el) => {
    while (document.body.contains(el)) {
        let cs = window.getComputedStyle(el), pos = { l: pxToNum(cs.left), r: pxToNum(cs.right), t: pxToNum(cs.top), b: pxToNum(cs.bottom) };
...
}, pressKeyCode = (keyCode, hasShift) => {
    var event = new KeyboardEvent('keydown', { keyCode: keyCode, shiftKey: hasShift, bubbles: true });
    document.activeElement.dispatchEvent(event);
    ...
    return el;
}, parseMineField = (container) => {
    const rows = Array.from(container.childNodes).slice(1, container.childNodes.length - 1);
    ...
    let matrix = [...Array(numRows)].map(e => Array(numColumns).fill(0));
    ...
    return matrix;
}, parseCyberpunkField = (elements) => {
    const dimensions = Math.sqrt(elements.length);
    let matrix = [...Array(dimensions)].map(e => {
...
main = async (ns) => {
    let 
    // --- Declarations --- //
    infilContainer = document.querySelector("#infiltration-container"), mainContent = `<p style="font-size:11px">Assistance:<input type=checkbox class=checkbox id="infil-assistance-toggle" checked /></p><p id="infil-helper-content"></p>`, infilWin = createWindow("infil", "Infiltration Helper", mainContent), helperContent = infilWin.querySelector("#infil-helper-content"), assistanceToggle = infilWin.querySelector("#infil-assistance-toggle"), changeContent = (content) => helperContent.innerHTML = content, getGameText = () => {
        try {
        ...
        while (getGameText() && assistanceToggle.checked) {
            switch (getGameText()) {
                case "Remember all the mines!":
                    let minefield = infilContainer.children[0].children[1].children[0].children[1];
                ...
                case "Match the symbols!":
                    ...
                    const targetStrings = targets.filter((element) => element.innerText != null).map((element) => element.innerText.replace(/\s/g, ''));
                ...
                case "Cut the wires with the following properties!":
                    ...
                    const wireElements = Array.from(infilContainer.children[0].children[1].querySelectorAll("span")).filter((element) => isNaN(+element.innerText));
                    const numWires = Array.from(infilContainer.children[0].children[1].querySelectorAll("span")).filter((element) => !isNaN(+element.innerText)).length;
                    const wires = [...Array(numWires)].map(e => {
                        return [];
                    });
